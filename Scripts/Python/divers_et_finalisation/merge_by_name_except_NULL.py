...
def fusionner_par_champ(champ_nom: str):
    ...
    # Regrouper uniquement les entités dont le champ n'est pas NULL
    groupes = {}
    for feat in couche.getFeatures():
        valeur = feat[champ_nom]
        if valeur is None:  # on ignore totalement les NULL
            continue
    ...
    for valeur, features in groupes.items():
        if len(features) < 2:
            continue  # rien à fusionner si une seule entité
    ...
    print("Fusion terminée (les entités NULL sont laissées intactes).")
